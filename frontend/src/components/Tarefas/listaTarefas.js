...
import TarefasOverlayTarefas from "../Overlays/tarefaOverlay";
...
function TarefasList({ tarefas }) {
  ...
  sortTarefas.forEach((tarefa) => {
    if (tarefa.operario_associado) {
      tarefa.operario_associado.forEach((operario) => {
        if (!countTarefas[operario]) {
          countTarefas[operario] = 0;
        }
        countTarefas[operario]++;
      });
    }
  });
            ...
            <tr key={tarefa.id} style={CorEstado({ tarefa })}>
              ...
              <td>
                {tarefa.operario_associado && (
                  <span>
                    {tarefa.operario_associado.reduce(
                      (total, operario) => total + countTarefas[operario],
                      0
                    )}
                  </span>
                )}
                <TarefasOverlayTarefas
                  operario_associado={tarefa.operario_associado}
                >
                  <span>
                    {tarefa.operario_associado}
                    <Info size={20} className="mx-1" />
